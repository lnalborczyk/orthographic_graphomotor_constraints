...
```{r data, echo = FALSE, fig.width = 10, fig.height = 8, out.width = "100%", fig.cap = "Effect of grade, word frequency, and grapheme complexity on the total duration, the number of stops, the mean velocity, and the letter size. The error bars represent the 95\\% confidence intervals of the mean (assuming a Gaussian distribution)."}
# importing the data
df <- read_excel(
...
```{r correlation, echo = FALSE, out.width = "100%", fig.width = 10, fig.height = 10, fig.cap = "Overall and by-grade Spearman correlation between each pair of measured variables."}
...
But it still can not account for sur-represented zero values in some groups (e.g., in the number of stops in Grade-2 and Grade-3 children, cf. Figure \ref{fig:data}). To fix this issue, we describe below a zero-inflated poisson regression model, where the number of stops can arise as the result of two distinct processes (i.e., a kind of mixture model). In our case, we consider that the number of stops may depend on writing expertise (for which grade is considered to be a proxy). Once writing is automatised, we do not expect stops. The probability of taping into this "zero-stop" process is given by the mixture probability $p_{i}$. This results in the following model:

$$
\begin{aligned}
y &\sim \mathrm{ZIPoisson}(p_{i}, \lambda_{i})\\
\text{logit}(p_{i}) &= \alpha_{p}\\
\log(\lambda_{i}) &= \alpha_{\lambda} + \beta_{\lambda g} \cdot \text{grade}_{i} + \beta_{f} \cdot \text{freq}_{i} +\\ &\quad  \beta_{graphe} \cdot \text{grapheme}_{i} + \beta_{grapho} \cdot \text{graphomotor}_{i}\\
\end{aligned}
$$

where the mixture probability $p_{i}$ is assigned its own intercept $\alpha_{p}$ (estimating the average probability of zero error).
...
```{r posterior-duration, echo = FALSE, eval = FALSE}
# Now that we have fitted the model, we are left with the task of interpreting the output from the model. The output of the model is a (joint) posterior distribution over all parameters of the model. We can marginalise this joint distribution to obtain the (marginal) posterior distribution on each parameter. To summarise this distribution, we can retrieve samples from the joint posterior distribution.

#############################################################################
# average predictions in the linear scale
######################################################################
...
duration_predictions <- df2 %>%
    ...
    ungroup %>%
    group_by(group, frequency, graphomotor_difficulty) %>%
    summarise(moy = mean(Estimate) )

# retrieves posterior samples (for all parameters related to letter duration)
posterior_samples <- posterior_samples(x = mod2, pars = regex("duration") ) %>%
    # keeps only slopes
    select(contains(match = "b_") )

# retrieves the posterior samples for the slope for frequency in Grade 1 for the letter "t"
duration_frequency_grade1_LF <- posterior_samples$b_duration_Intercept +
    (-0.5) * posterior_samples$b_duration_frequency1 +
    (-0.5) * posterior_samples$b_duration_graphomotor_difficulty1 +
    (-0.5) * (-0.5) * posterior_samples$`b_duration_frequency1:graphomotor_difficulty1`

# retrieves the posterior samples for the slope for frequency in Grade 1 for the letter "f"
duration_frequency_grade1_HF <- posterior_samples$b_duration_Intercept +
    0.5 * posterior_samples$b_duration_frequency1 +
    (-0.5) * posterior_samples$b_duration_graphomotor_difficulty1 +
    0.5 * (-0.5) * posterior_samples$`b_duration_frequency1:graphomotor_difficulty1`

# computes slope for frequency for Grade 1 and easy letter (t)
slope_t <- duration_frequency_grade1_HF - duration_frequency_grade1_LF
...
# computes slope for frequency for Grade 1 and hard letter (f)
slope_f <- duration_frequency_grade1_HF - duration_frequency_grade1_LF

########################
# for grade 3
####################

# retrieves the posterior samples for the slope for frequency in Grade 3 for the letter "t"
duration_frequency_grade3_LF <- posterior_samples$b_duration_Intercept +
    posterior_samples$b_duration_groupGrade3 +
    (-0.5) * posterior_samples$b_duration_frequency1 +
    (-0.5) * posterior_samples$b_duration_graphomotor_difficulty1 +
    (-0.5) * (-0.5) * posterior_samples$`b_duration_frequency1:graphomotor_difficulty1` +
    (-0.5) * posterior_samples$`b_duration_groupGrade3:frequency1` +
    (-0.5) * posterior_samples$`b_duration_groupGrade3:graphomotor_difficulty1` +
    (-0.5) * (-0.5) * posterior_samples$`b_duration_groupGrade3:frequency1:graphomotor_difficulty1`
...
plotPost(duration_frequency_grade3_LF)

# computes slope for frequency for Grade 3 and easy letter (t)
slope_t <- duration_frequency_grade3_HF - duration_frequency_grade3_LF
...
plotPost(
    paramSampleVec = slope_t,
    showMode = FALSE, compVal = 0,
    xlab = expression(paste(beta[frequency][paste("[", Grade1-t, "]")] ) )
    )

plotPost(
    paramSampleVec = slope_f,
    showMode = FALSE, compVal = 0,
    xlab = expression(paste(beta[frequency][paste("[", Grade1-f, "]")] ) )
    )
...
```{r posterior-slopes-drift, echo = FALSE, eval = FALSE, out.width = "100%", fig.asp = 0.5, fig.cap = "Posterior distribution of the slopes for the drift rate, for Group, Prime, and Cong (respectively). The mode (i.e., the most probable value) and the 95\\% credible interval are also displayed."}
...
```{r posterior-bias-difference, echo = FALSE, eval = FALSE, fig.asp = 1, out.width = "50%", fig.cap = "Posterior distribution of the difference between the estimated starting point in the ASD group and the estimated starting point in the TD group."}
...
```{r posterior-bias-prime, echo = FALSE, eval = FALSE, fig.asp = 1, out.width = "50%", fig.cap = "Posterior distribution of the odds ratio between LSF and HSF primes on the starting point."}
# plotting it
par(mfrow = c(1, 1) )
plotPost(
